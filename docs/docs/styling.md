...
Styled-Components is currently required as a peer dependency but I don't know if that will last forever :shrug: There are a few classNames included on key components such as `rsm-arrow-button`, `rsm-confirm-button`, `rsm-cancel-button`, and `rsm-date-title` for _relatively_ easy CSS selection.
